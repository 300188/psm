...
<form class="form-inline" role="form" style="float:left;">
  <div class="form-group has-feedback">
    <div class="input-group">
      <div class="input-group-addon">查询日志内容</div>
            	...
            	$.ajax({
            		...
            		success : function(msg) {
            			...
            			if ( result.success ) {
            				...
            				var logPage = result.data;
            				var logs = logPage.datas;
            				
            				$.each(logs, function(i, log){
                                var now = new Date();
                                var Time = new Date(parseInt(log.createTime)) ;
                                var commonTime = Time.toLocaleString();

                                tableContent += '<tr>';
	          	                tableContent += '  <td>'+(i+1)+'</td>';
	          					tableContent += '  <td><input type="checkbox" name="logid" value="'+log.id+'"></td>';
	          	                tableContent += '  <td>'+log.log+'</td>';
	          	                tableContent += '  <td>'+commonTime+'</td>';
	          	                tableContent += '  <td>';
	          					tableContent += '	  <button type="button" onclick="deleteRecord('+log.id+', \''+log.log+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
	          					tableContent += '  </td>';
	          	                tableContent += '</tr>';
            				});
            				...
            				for ( var i = 1; i <= logPage.totalno; i++ ) {
            					if ( i == pageno ) {
